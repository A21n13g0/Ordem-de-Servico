...
#Label top
app_name = Label(frame_top, text='Formulario de Ordem de Serviço', anchor=NW, font=('Ivy 13 bold'), bg=co2, fg=co1, relief='flat')
app_name.place(x=10, y=20)

#Label base
label_os = Label(frame_base, text='Numero da OS *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
label_os.place(x=10, y=10)
entry_os = Entry(frame_base, width=45, justify='left', relief='solid')
entry_os.place(x=15, y=30)

label_typeService = Label(frame_base, text='Tipo de Serviço *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
...
label_description = Label(frame_base, text='Descrição *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
...
label_date = Label(frame_base, text='Data do Serviço *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
label_date.place(x=10, y=140)
entry_date = DateEntry(frame_base, width=12, background='darkblue', foreground='white', borderwidth=2)
entry_date.place(x=15, y=160)

label_provider = Label(frame_base, text='Prestador *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
...
label_client = Label(frame_base, text='Cliente *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
...
label_value = Label(frame_base, text='Valor *', anchor=NW, font=('Ivy 10 bold'), bg=co1, fg=co4, relief='flat')
...
def show():
    ...
    table_head = ['ID','OS', 'Serviço', 'Descrição','Data', 'Provedor', 'Cliente', 'Valor']

    #Criando a tabela
    tree = ttk.Treeview(frame_right, selectmode="extended", columns=table_head, show="headings")
...
def change():
    ...
    try:
        ...
        def executeUpdate():
            ...
            if os == '' or typeService == '' or description == '' or provider == '' or client == '' or value == '':
                messagebox.showerror("Erro", "Preencha todos os campos para atualizar!")
            else:
                aux = 0
                service = [os, typeService, description, date, provider, client, value, id]
                update(service)
        ...
        btn_confirm = Button(frame_base, command=executeUpdate, text='Confirmar', width=7, font=('Ivy 10 bold'), bg=co8, fg=co1, relief='raised', overrelief='ridge')
...
def remove():
    try:
        ...
        delete(id)
        messagebox.showinfo("Sucesso", "Ordem de serviço removida com sucesso!")
        ...
        show()
            ...
            messagebox.showerror("Erro", "Selecione um registro da tabela!")
#Botões
btn_insert = Button(frame_base, command=register, text='Inserir', width=7, font=('Ivy 10 bold'), bg=co2, fg=co1, relief='raised', overrelief='ridge')
btn_insert.place(x=10, y=350)

btn_update = Button(frame_base, command=change, text='Atualizar', width=7, font=('Ivy 10 bold'), bg=co8, fg=co1, relief='raised', overrelief='ridge')
btn_update.place(x=85, y=350)

btn_delete = Button(frame_base, command=remove, text='Deletar', width=7, font=('Ivy 10 bold'), bg=co7, fg=co1, relief='raised', overrelief='ridge')
